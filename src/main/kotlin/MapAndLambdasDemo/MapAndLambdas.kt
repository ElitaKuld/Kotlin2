...
fun main(){
    ...
    //gruppering skapar values som är listor, där originalobjekten ligger
    val grEx = allPersonList.groupBy(Person::city);
    println(grEx.map { "${it.key}: ${it.value}" })
    println(grEx.map { "${it.key}: ${it.value.map{it.name}}" }) // vi använder map för att skriva ut staden(key) och namnet på personen(value)
    ...
    val assWithEx = allPersonList.associateWith { it.name } // fungerar bara om vi har unika värden att associera med
